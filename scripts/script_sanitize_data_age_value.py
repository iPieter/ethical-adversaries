...
reduced_col_data=raw_data.filter(items=['sex','age','race','juv_fel_count','juv_misd_count','juv_other_count','priors_count','days_b_screening_arrest','c_jail_in','c_jail_out','c_charge_degree','is_recid','decile_score','score_text','two_year_recid'])
...
reduced_data.to_csv("../data/csv/compas_recidive_two_years_sanitize_age_value.csv",index=False,header=True)
...
print(reduced_data.loc[(reduced_data['age'] >= 25) & (reduced_data['age'] < 45)].count()[0])
print(reduced_data.loc[reduced_data['age'] >= 45].count()[0])
print(reduced_data.loc[reduced_data['age'] < 25].count()[0])
